...
public class BoggleSolver
{
		...
		for(int direction=0; direction<8; direction++){
			if(isValid(row, col, direction)){
				...
				if(res == 3){ //word and prefix
					//: Write the code for the word and prefix case
					if(currentSolution.length() >= 3){
						System.out.println(currentSolution.toString());
					}
					solve(nextCoords.row, nextCoords.col, depth + 1);
	...
	private boolean isValid(int row, int col, int direction){
		...
		if(coords.row<0||coords.row>=4||coords.col<0||coords.col>=4) return false;
		if(theBoard[coords.row][coords.col]-'A'<=25&&theBoard[coords.row][coords.col]-'A'>=0) return false;
		return true;
		//: Write the code to check whether already used / at an edge of the board
	}
