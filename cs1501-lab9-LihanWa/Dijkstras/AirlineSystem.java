  ...
  private void shortestHops() {
    if(G == null){
      ...
      if(!G.marked[destination]){
        ...
        while(!b.empty()){
          w.append(cityNames[b.pop()]);
          w.append(" ");
          
        }
        System.out.print(" The shortest route from "+cityNames[source]+" to "+cityNames[destination]+" has "+cnt+" hop(s): "+w.toString()+"\n"); 
  ...
  private class Digraph {
    ...
    */
	...
	//: Update marked[] and nMarked. Check for disconnected graph.
        if(current!=-1) marked[current]=true;
        else break;
